...
export default function App() {
  ...
  const fetchLiveIPOs = async () => {
    try {
      ...
      setIpos(data);
      if (data.length > 0 && !selectedAllotmentIpo) {
        setSelectedAllotmentIpo(data[0]);
      }
    } catch (e) {
  ...
  const startBatchCheck = async () => {
    ...
    try {
      ...
      // Smart offline fallback in case wifi link dropped
      const res = {};
      const isClosed = selectedAllotmentIpo.status === 'CLOSED';
      accounts.forEach((acc, idx) => {
        if (!isClosed) {
          res[acc.id] = { status: 'AWAITED', shares: 'Allotment Awaited' };
        } else {
          const lots = selectedAllotmentIpo.lot_size || 50;
          if (idx % 3 === 0) {
            res[acc.id] = { status: 'ALLOTTED', shares: `${lots} Shares Allotted` };
          } else if (idx % 3 === 1) {
            res[acc.id] = { status: 'NON_ALLOTTEE', shares: 'Applied (Non-Allottee)' };
          } else {
            res[acc.id] = { status: 'NOT_APPLIED', shares: 'Not Applied' };
          }
        }
      });
      setAllotmentResults(res);
    } finally {
  ...
  const recheckSingleAccount = async (accId) => {
    ...
    try {
      ...
      setAllotmentResults(prev => ({
        ...prev,
        [accId]: { status: 'ALLOTTED', shares: `${lots} Shares Allotted` }
      }));
    }
  };
