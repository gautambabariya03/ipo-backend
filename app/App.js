...
export default function App() {
  ...
  const [showMainboard, setShowMainboard] = useState(true);
  ...
  const smartDisplayList = (ipos || []).filter(item => {
    // Agar issue size ya GMP '--' ya 'TBA' hai, toh ise UPCOMING maano
    const isMissingData = !item.issue_size || String(item.issue_size).includes('--') || String(item.issue_size).includes('TBA');
    let effectiveStatus = item.status || 'OPEN';
    if (effectiveStatus === 'OPEN' && isMissingData) {
      effectiveStatus = 'UPCOMING';
    }
    
    // Status filter match
    const matchStatus = typeof statusFilter !== 'undefined' ? (effectiveStatus === statusFilter) : true;
    
    // Mainboard / SME match
    const isSME = String(item.name).toLowerCase().includes('sme');
    const matchType = (isSME && showSME) || (!isSME && showMainboard);
    
    // Search match
    const matchSearch = searchQuery === '' || String(item.name).toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <Modal visible={detailModalOpen} transparent animationType="slide">
        ...
        {/* --- ALLOTMENT PROGRESS UI --- */}
        {activeTab === 'Allotment' && checkingProgress !== '' && (
          <View style={{ backgroundColor: '#eff6ff', padding: 10, marginHorizontal: 15, borderRadius: 8, marginBottom: 10, borderWidth: 1, borderColor: '#bfdbfe' }}>
            <Text style={{ color: '#2563eb', fontWeight: 'bold', textAlign: 'center', fontSize: 15 }}>{checkingProgress}</Text>
          </View>
        )}
        ...
        {activeTab === 'IPO' && (
          <View style={{ paddingHorizontal: 15, paddingBottom: 5 }}>
            {isSearchActive ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#f1f5f9', borderRadius: 8, paddingHorizontal: 10, marginBottom: 10, borderWidth: 1, borderColor: '#cbd5e1' }}>
                <TextInput style={{ flex: 1, paddingVertical: 8, color: '#000', fontSize: 16 }} placeholder="Search IPOs..." placeholderTextColor="#64748b" value={searchQuery} onChangeText={setSearchQuery} autoFocus />
                <Text onPress={() => { setIsSearchActive(false); setSearchQuery(''); }} style={{ color: '#ef4444', fontWeight: 'bold', padding: 10, fontSize: 18 }}>X</Text>
              </View>
            ) : (
              <Text onPress={() => setIsSearchActive(true)} style={{ backgroundColor: '#e2e8f0', padding: 10, borderRadius: 8, textAlign: 'center', marginBottom: 10, color: '#475569', fontWeight: 'bold', fontSize: 15 }}>
                🔍 Tap to Search IPOs
              </Text>
            )}
            <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginBottom: 5 }}>
              <Text onPress={() => setShowMainboard(!showMainboard)} style={{ fontSize: 14, fontWeight: 'bold', color: showMainboard ? '#3b82f6' : '#94a3b8' }}>
                {showMainboard ? '☑️' : '⬜'} MAINBOARD
              </Text>
              <Text onPress={() => setShowSME(!showSME)} style={{ fontSize: 14, fontWeight: 'bold', color: showSME ? '#3b82f6' : '#94a3b8' }}>
                {showSME ? '☑️' : '⬜'} SME
              </Text>
            </View>
          </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={[styles.darkTable, { borderColor: theme.border }]}>
                ...
                <View style={[styles.darkTableRow, { borderColor: theme.border }]}><Text style={[styles.tableTxt, { color: theme.textSub }]}>QIB (Institutional)</Text><Text style={[styles.tableTxtBold, { color: theme.textMain }]}>{selectedIpo?.subscription?.qib}</Text></View>
                <View style={[styles.darkTableRow, { borderColor: theme.border }]}><Text style={[styles.tableTxt, { color: theme.textSub }]}>HNI / NII</Text><Text style={[styles.tableTxtBold, { color: theme.textMain }]}>{selectedIpo?.subscription?.hni}</Text></View>
                <View style={[styles.darkTableRow, { borderColor: theme.border }]}><Text style={[styles.tableTxt, { color: theme.textSub }]}>Retail Public</Text><Text style={[styles.tableTxtBold, { color: theme.textMain }]}>{selectedIpo?.subscription?.retail}</Text></View>
                <View style={[styles.darkTableRow, { backgroundColor: theme.terminalBg, borderColor: theme.border }]}><Text style={[styles.tableTxtBold, { color: '#10B981' }]}>Total Subscription</Text><Text style={[styles.tableTxtBold, { color: '#10B981' }]}>{selectedIpo?.subscription?.total}</Text></View>
              </View>
